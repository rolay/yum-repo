...
RUN mkdir /rpms/kylinv10-sp3/ -p && \
    ...
    yumdownloader --resolve --forcearch=aarch64 --destdir=/rpms/kylinv10-sp3 audit audit-libs-python3 bash-completion bind-utils curl chrony conntrack fuse git graphviz ipset ipvsadm iscsi-initiator-utils jq kubernetes-cni nfs-utils nc net-tools python3 python3-chardet python3-docker python3-pip python3-requests sysstat tar tcpdump && \
    yumdownloader --resolve --forcearch=aarch64 --destdir=/rpms/kylinv10-sp3 podman cri-o container-selinux policycoreutils-python-utils python3-policycoreutils python3-libsemanage && \
    yumdownloader --resolve --forcearch=aarch64 --destdir=/rpms/kylinv10-sp3 kubectl kubelet kubeadm
